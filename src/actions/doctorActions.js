...
export const listDoctors = () => async (dispatch) => {
    try {
        ...
        // Fetch doctors along with their associated user's first name
        const { data, error } = await supabase
            .from('doctors')
            .select(`
                *,
                custom_users (first_name, last_name)  // Ensure this matches your relationship name
            `);

        if (error) throw error;

        // Map the data to include the first name in the result
        const doctorsWithNames = data.map(doctor => ({
            ...doctor,
            first_name: doctor.custom_users.first_name,
            last_name: doctor.custom_users.last_name // Add last name if needed
        }));

        dispatch({
            type: DOCTOR_LIST_SUCCESS,
            payload: doctorsWithNames,
        });
    } catch (error) {
        dispatch({
            type: DOCTOR_LIST_FAIL,
            payload: error.message || 'Failed to fetch doctors.',
        });
    }
};
